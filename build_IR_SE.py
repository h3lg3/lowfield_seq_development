...
## Calculate timing
te_fill1 = TE/2 - (
    pp.calc_duration(gz_ex)/2 + pp.calc_duration(max_gy, gzr_ex, gx_pre) + pp.calc_duration(gx_sp, gz_sp_pre) + pp.calc_duration(gz_ref)/2
    )
te_fill2 = TE/2 - (
    pp.calc_duration(gz_ref)/2 + pp.calc_duration(gx_sp, gz_sp_post) + pp.calc_duration(gx)/2
    )

tr_fill= TR - (
    pp.calc_duration(gx_sp, max_gy_rew) + pp.calc_duration(gz_ex)/2 + TE + pp.calc_duration(gx)/2
    )
assert(te_fill1 >= 0)
assert(te_fill2 >= 0)
assert(tr_fill >= 0)
