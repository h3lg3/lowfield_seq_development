...
gx = pp.make_trapezoid(channel='x', flat_area=delta_k*n_read, flat_time=ro_duration, system=system)
gx_pre = pp.make_trapezoid(channel='x', area=gx.area/2, duration=ro_duration/2, system=system)
...
gx_sp = pp.make_trapezoid(channel='x', area=1.5*gx.area, duration=ro_duration, system=system)

## calculate phase encode gradient
######### AREA oder FLAT AREA? 
max_gy = pp.make_trapezoid(channel='y', area=delta_k*n_phase/2, duration=pp.calc_duration(gx_pre), system=system)
#max_gy = pp.make_trapezoid(channel='y', flat_area=delta_k*n_phase/2, flat_time=pe_duration, system=system)
scale_gy = np.linspace(0, n_phase-1, n_phase) - n_phase/2
...
adc = pp.make_adc(num_samples=n_read, duration=ro_duration, phase_offset=180*np.pi/180, delay=gx.rise_time, system=system)

## Without x and y gradients
# te_fill1 = TE/2-rf_ex.shape_dur/2-rf_ex.ringdown_time-rf_ref.shape_dur/2-rf_ref.delay
# te_fill2 = TE/2-rf_ref.shape_dur/2-rf_ref.ringdown_time-adc.delay-(adc.num_samples*adc.dwell)/2

## With x and y gradients
te_fill1 = TE/2 - (pp.calc_duration(gz_ex)/2 + pp.calc_duration(max_gy, gzr_ex, gx_pre) + pp.calc_duration(gx_sp) + pp.calc_duration(gz_ref)/2)
te_fill2 = TE/2 - (pp.calc_duration(gz_ref)/2 + pp.calc_duration(gx_sp) + pp.calc_duration(gx)/2)
tr_fill= TR - (pp.calc_duration(gx_sp) + pp.calc_duration(gz_ex)/2 + TE + pp.calc_duration(gx)/2)
assert(te_fill1 >= 0)
assert(te_fill2 >= 0)
assert(tr_fill >= 0)
...
for i in range(n_phase): 
    ...
    gy = pp.scale_grad(max_gy, scale_gy[i])
