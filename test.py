...
gx_sp = pp.make_trapezoid(
    channel='x', area=1.5*gx.area, duration=ro_duration, system=system
    )
...
gz_sp_pre = pp.make_trapezoid(channel='z', area=1000,  system=system)
gz_sp_pre.delay = pp.calc_duration(gx_sp) - pp.calc_duration(gz_sp_pre)
gz_sp_post = pp.make_trapezoid(channel='z', area=1000,  system=system)

## Set gradients=0 and nphase=1 to test for echo symmetry
# n_phase = 1
max_gy.amplitude = 0
max_gy_rew.amplitude = 0
# gx.amplitude = 0
# gx_pre.amplitude = 0
gx_sp.amplitude = 0
...
## Calculate timing
te_fill1 = TE/2 - (pp.calc_duration(gz_ex)/2 + pp.calc_duration(max_gy, gzr_ex, gx_pre) + pp.calc_duration(gx_sp, gz_sp_pre) + pp.calc_duration(gz_ref)/2)
te_fill2 = TE/2 - (pp.calc_duration(gz_ref)/2 + pp.calc_duration(gx_sp, gz_sp_post) + pp.calc_duration(gx)/2)

tr_fill= TR - (pp.calc_duration(gx_sp, max_gy_rew) + pp.calc_duration(gz_ex)/2 + TE + pp.calc_duration(gx)/2)
assert(te_fill1 >= 0)
assert(te_fill2 >= 0)
assert(tr_fill >= 0)
...
for i in range(n_phase): 
    ...
    gy_rew = pp.scale_grad(max_gy_rew, scale_gy[i])    
